...
| LLM-005 Build versioned prompt and response registry | **Server registry implemented** (commit pending); agent PromptRegistry already existed | `PromptRegistry` in `agent/src/prompts.rs` loads TOML templates. Missing: JSON schema validation, evaluation corpus, server-side registry. |
...
| LLM-020 Correct documentation and product claims | **Substantially done** — docs/CAPABILITY_MATURITY.md (229 lines, WP-025) provides the five-dimension maturity framework + honest labels | `CLAUDE.md` updated. `docs/LLM_CAPABILITY_CONTRACT.md` written. Missing: `README.md` capability-maturity section, per-feature experimental/supported labels. |
...
| **UX-009** No responsive fallback for fleet tables | UX | **Implemented** | Inventory/coverage tables force `min-w-[1200px]` horizontal scroll on small screens; add column-priority or card layout at narrow widths. |
