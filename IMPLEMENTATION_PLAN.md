...
| **UX-006** Configure modal has no focus trap | A11y | **To do** | The login modal uses `FocusTrap`; the per-agent Configure modal does not — focus isn't trapped and there's no Escape handler. Add both. |
...
| **UX-014** Long tables lack sticky headers | UX | **Partial** | Added to the honest-coverage table (UX-016). `FindingsGrid` and `AgentFleetInventory` tables still scroll their headers off; apply sticky `thead` there too. |
