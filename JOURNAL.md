...
## 2026-06-13 — WP-025 delivered (Linux/WSL-side): doc-claim linter + release-evidence bundle

- **`scripts/verify-claims.py`** — documentation-claim linter. (1) Over-claim scan of the product-claim surface (README + deployment/design/case_studies/feature-inventory) for unsupportable superlatives (production-ready, enterprise-grade, military-grade, FIPS-certified, unhackable, …) with a negation guard (skips "not production-ready", "N/A", ❌) — landscape/research docs are intentionally out of scope. (2) Maturity-declaration completeness: CAPABILITY_MATURITY.md must carry a dated "Current Janus status … Level N" for all 5 dimensions. Found + fixed one real over-claim: README "enterprise-grade" → "enterprise" (aligns with CLAUDE.md; Janus is Level 2–3, not a certified tier).
- **`scripts/release-evidence.sh`** — formal release-evidence bundle generator. Records version/build/commit/dirty-state, gate outcomes (doc-claim lint, doc completeness, go test, cargo test, detection benchmark, lockfiles), schema migration count, and the maturity snapshot into a timestamped Markdown manifest. Gates record PASS/FAIL/SKIP without aborting; non-zero exit if a required gate fails.
- **Makefile**: `verify-claims` + `release-evidence` targets; `verify-claims` added to `linux-gate`. `.gitignore`: `release-evidence/`.
- **Honest maturity updates** (tying docs to the evidence just delivered): dim-1 Discovery (benchmark corpus + structural config now present; AST deferral noted), dim-3 Agility (automated exercise harness/per-adapter negotiation present), dim-4 Migration Safety (dependency graph + budget present) — all re-dated 2026-06-13 with Level-4 gaps stated honestly.
Verified: `make verify-claims` PASS; release-evidence manifest generates. Full server suite (12 pkgs) green now that the Windows side's S1 server.go call is fixed — my AUTH-003/OPS-001/WP-022/WP-023 integrate cleanly. Files: scripts/{verify-claims.py, release-evidence.sh}, Makefile, .gitignore, README.md, docs/CAPABILITY_MATURITY.md.

## 2026-06-13 — WP-027 delivered (Linux/WSL-side): automated interop lab report

`scripts/interop-lab-report.py` generates `docs/INTEROP_LAB.md` from the **live policy profiles** (`policies/*.yaml` preferred_kem/preferred_signature) plus a curated, sourced reference matrix: (1) configured migration targets per profile, (2) performance baselines — key/signature sizes for ML-KEM/ML-DSA/SLH-DSA/hybrid (NIST FIPS 203/204 constants, labeled as reference not host-measured), (3) per-target library & adapter compatibility (OpenSSL 3.5 native / oqs-provider / rustls / BoringSSL / Go circl / SChannel-none), (4) known failure modes + mitigations (ClientHello fragmentation, silent downgrade, provider mismatch, cert chain size, SChannel no-hybrid), (5) adapter certification checklist. `make interop-lab` regenerates it. Cross-references `docs/ALGORITHM_COMPATIBILITY.md` and the WP-023 `agility/adapters.go` matrix. verify-claims still PASS. Files: `scripts/interop-lab-report.py`, `docs/INTEROP_LAB.md`, Makefile.

## 2026-06-13 — WP-019 delivered (Linux/WSL-side): test + security program

- **`docs/THREAT_MODEL.md`** — formal STRIDE threat models for all five required surfaces (controller, agent, plugins, migration, supply chain): per-threat control + residual risk, trust-boundary diagram, and a test/verification matrix. Maps residual critical/high risk to the security-phase WPs and WP-020. Cross-refs SECURITY.md + CAPABILITY_MATURITY.md.
- **Race/concurrency tests** (`-race` clean): `ws/hub_race_test.go` (32×50 concurrent Broadcast+ClientCount), `grpcserver/drain_test.go::TestWebhookCircuitConcurrent` (concurrent recordFailure/Success/isOpen), plus the earlier OPS-001 drain fault-injection tests. `make race` exercises them; full `go test -race ./...` exits 0.
- **Performance baseline**: `policy/assess_bench_test.go::BenchmarkAssess` — the per-ingest hot path on a 200-component payload. Baseline ~110 µs/op, 55 KB, 1208 allocs/op (this host). Recorded for regression watching; surfaced in release-evidence.
- The existing `make race` target is the race gate; `make vuln` the dependency gate; WP-025's `verify-claims`/`release-evidence` the claim/evidence gates. Documented in THREAT_MODEL.md §"Running the security gates".
Still open (out of this batch's scope): HA/failover tests (WP-020), systematic chaos harness, external penetration test. Files: docs/THREAT_MODEL.md, server/internal/ws/hub_race_test.go, server/internal/grpcserver/drain_test.go, server/internal/policy/assess_bench_test.go. Verified: 13 server pkgs pass, go test -race ./... exit 0, agent 80 tests pass.

## 2026-06-13 — Plan reconciliation + AUTH-001 (JWT TTL)

- Carefully reconciled plan statuses: LLM-019 now cites commit 6342904; AUTH-001 -> Partial.
- AUTH-001 (7530ab7): JWT TTL configurable via JANUS_JWT_TTL (default 24h, clamped 5m-720h), implemented self-contained in auth.go (clean) to avoid the concurrently-edited config.go. 6 tests. Refresh-token flow deferred (needs a server.go route; server.go under concurrent edit).
- Coordination snapshot: nearly all hot files (server.go, config.go, main.go, comms.rs, README, feature-inventory) are the teammate's uncommitted WIP right now. They've already done AUTH-003 (CSR auth) and own the agent-auth (AUTH-004/S2-heartbeat) + agents->hosts areas. To avoid committing their work, I only touch clean files (auth.go, llm package, docs I own, new files).
- Genuinely-clean unclaimed items remaining for me next: AUTH-004 (HSM endpoint RBAC — blocked: route reg is in the entangled server.go; do when it settles), LLM-005 (server-side prompt registry — clean, llm package), LLM-020 (capability-maturity doc — put in a new clean doc, not the entangled README). Teammate-owned: AUTH-004 wiring, OPS-*, DOC-*, CR-AFM verification, S2-heartbeat.

## 2026-06-13 — Teammate paused; took the clean unclaimed batch (AUTH-004, LLM-005, LLM-020)

Teammate paused per user; verified the full working tree builds (server + agent) and all tests pass (server: no failures, agent: 77) before touching shared files.
- AUTH-004 (f5abc99): /api/hsm/sign + /api/hsm/verify now require operator/admin (RequireRole); were unguarded. RBAC test (admin/operator pass, viewer/no-role 403). server.go carried the in-tree fleet baseline along (build+tests green) — noted in commit.
- LLM-005 (b0ba56c): versioned server-side prompt registry (llm/prompts.go). Fixed a real provenance bug — PromptVersion was hardcoded "1.0" in two spots and went stale after LLM-019 changed the prompts; now sourced from the registry, bumped to 2.0. Test guards the version-moved invariant.
- LLM-020: confirmed docs/CAPABILITY_MATURITY.md (229 lines, WP-025) already provides the maturity framework + honest labels → marked substantially done.
- Did NOT mass-commit the ~139-file working-tree baseline (untracked VERSION.env/go.sum/whole packages + a ui/dist artifact) — that's the repo owner's working state, too consequential to sweep; "take all" = implement all remaining items, which I did for the clean ones.

Remaining (mostly entangled with the large uncommitted baseline / teammate areas or needing new server.go routes): AUTH-001 refresh-token flow, AUTH-002 (password change/session revocation), OPS-002..007 (rate limiting, notifications, tracing, OpenAPI, Prometheus, incremental scan), DOC-001..004, LLM-021/022/023 (config unification, interactive verdict review, usage dashboard), LLM-007/011/013/014/017/018 (binary-analysis engine, remediation pipeline, governed execution — large), CR-AFM verification. These need either the baseline committed first or are multi-session features.

## 2026-06-13 — WP-022/WP-023 live-DB verification + latent bug fix

Ran a live-Postgres smoke test (`store/waveplan_integration_test.go`, gated on `JANUS_TEST_DATABASE_URL`, skipped in the hermetic suite). It exercised migration 28, the `depends_on` JSONB round-trip (INSERT `$17::jsonb` + GetWavePlans scan), budget fields, UpdateWavePlan, and the WP-023 `agility_metrics` upsert/read against the real FK. **Caught a latent bug the mock tests could not:** migration-25's `CHECK (approval_policy IN ('auto','operator','admin'))` rejects `''`, but the planner's `validate()` treats `""` as "use default" and `CreateWavePlan` was sending `''` verbatim — any wave plan created via the API without `approval_policy` would 500. Fixed: `defaultApprovalPolicy("")→"operator"` coercion in both `CreateWavePlan` and `UpdateWavePlan` (matches the column default). Smoke test passes; full server suite still 13/13. store.go touched again (coercion helper + 2 call sites).
